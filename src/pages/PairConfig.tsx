...
const PairConfig: React.FC = () => {
  ...
  const handleSavePair = async () => {
    ...
    try {
      ...
      if (!editedPair.NO) {
        const createdPair = await tradingPairConfigAccess.create(editedPair);
        savedPairNo = createdPair.NO || 0;
        
        // 获取新创建的完整记录
        const newPair = tradingPairConfigAccess.getByNo ? 
          await tradingPairConfigAccess.getByNo(savedPairNo) :
          await tradingPairConfigAccess.get(savedPairNo);
        if (newPair) {
        ...
        // 更新现有交易对
        await tradingPairConfigAccess.update(editedPair);
  ...
  const handleChainConfigChange = (index: number, field: string, value: string | boolean) => {
    if (editedPair) {
      const updatedPairList = editedPair.pairList.map((config, i) => {
        ...
        // 如果更新的是tokenId字段，同时更新对应的token名称
        if (field === 'token1Id') {
  ...
  const handleToggleStatus = async (pair: TradingPairConfigModel) => {
    ...
    try {
      ...
      // 更新数据库
      await tradingPairConfigAccess.update(updatedPair);
          ...
          <PairList>
            ...
            <PairItems>
              {filteredPairs.length > 0 ? (
                ...
                <EmptyMessage>没有找到匹配的交易对</EmptyMessage>
              )}
            </PairItems>
          </PairList>
