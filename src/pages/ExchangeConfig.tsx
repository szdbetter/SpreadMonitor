...
const ExchangeConfig: React.FC = () => {
  ...
  const handleSaveExchange = async () => {
    ...
    try {
      ...
      if (!editedExchange.NO) {
        const createdExchange = await exchangeConfigAccess.create(editedExchange);
        savedExchangeNo = createdExchange.NO || 0;
        ...
        // 如果是编辑现有交易所，则更新记录
        await exchangeConfigAccess.update(editedExchange);
  ...
  const handleToggleStatus = async (exchange: ExchangeConfigModel) => {
    ...
    try {
      ...
      await exchangeConfigAccess.update(updatedExchange);
