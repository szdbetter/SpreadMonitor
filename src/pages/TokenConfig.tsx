...
const TokenConfig: React.FC = () => {
  ...
  const handleSaveToken = async () => {
    ...
    try {
      ...
      if (!editedToken.NO) {
        const createdToken = await tokenConfigAccess.create(editedToken);
        savedTokenNo = createdToken.NO || 0;
        ...
        // 如果是编辑现有Token，则更新记录
        await tokenConfigAccess.update(editedToken);
  ...
  const handleToggleStatus = async (token: TokenConfigModel) => {
    ...
    try {
      ...
      await tokenConfigAccess.update(updatedToken);
