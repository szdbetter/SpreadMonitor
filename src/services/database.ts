// 导入适配器工厂和存储管理器
import { DataFactory } from './adapters/dataFactory';
import { getCurrentStorageType, addStorageTypeListener, setupOfflineDetection } from './adapters/storageManager';
...
// 使用工厂方法创建适配器，替换原有的直接实例化
export const chainConfigAccess = DataFactory.getAdapter<ChainConfigModel>("ChainConfig", currentStorageType);
export const tokenConfigAccess = DataFactory.getAdapter<TokenConfigModel>("TokenConfig", currentStorageType);
export const tradingPairConfigAccess = DataFactory.getAdapter<TradingPairConfigModel>("TradingPairConfig", currentStorageType);
export const exchangeConfigAccess = DataFactory.getAdapter<ExchangeConfigModel>("ExchangeConfig", currentStorageType);
export const apiConfigAccess = DataFactory.getAdapter<ApiConfigModel>("ApiConfig", currentStorageType);
export const alertConfigAccess = DataFactory.getAdapter<AlertConfigModel>("AlertConfig", currentStorageType);

// 初始化离线检测
setupOfflineDetection();

// 添加存储类型变更监听
addStorageTypeListener((newType) => {
  console.log('存储类型已变更:', newType);
  // 在实际应用中，你可能需要刷新页面或重新加载数据
});
